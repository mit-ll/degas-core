classdef IntegrationTestClass < Simulation
% Copyright 2008 - 2020, MIT Lincoln Laboratory
% SPDX-License-Identifier: X11
%
% INTEGRATIONTESTCLASS Simulation that tests whether components are
% integrated correctly
    ...
    properties
        ...
        % specific block that they are associated with. For the
        % IntegrationTestClass object, consider these parameters to be
        % fixed for the integration test
        ...
        uasSensorSelected          = 1;   % Which sensor to test?
        ...
        simSpecificVars = {'ignorePrevAlert', 'enableVertMan', 'enableHorzMan',...
                           ...
                           'sim_sampleTime', 'uasSensorSelected'};        
        ...
        % Sensor models
        ac1OwnSense @ PerfectSurveillance
        ac1IntSensePerf @ PerfectSurveillance
        ac1IntSenseSimpTrkdSurv @ SimpleTrackedSurveillance
        ac1IntSenseEOIR @ EOIRParametricModel 
        ac1IntSenseADSB @ SC228_AdsbModel 
        ac1IntSenseTrackedADSB @ SC228_TrackedAdsbModel
        ac1IntSenseAS @ SC228_ActiveSurveillanceModel
        ac1IntSenseTrackedAS @ SC228_TrackedActiveSurveillanceModel
        ac1IntSenseRadar @ SC228_RadarModel
        ac1IntSenseTrackedRadar @ SC228_TrackedRadarModel
    ...
    methods        
        function this = IntegrationTestClass() % Constructor
            this = this@Simulation('IntegrationTestSim');
            ...
            this.ac1IntSensePerf = PerfectSurveillance('ac1IntPerfSurv_', 'surveillanceSampletime_s', this.uasSurveillance_sampleTime );
            this.ac1IntSenseSimpTrkdSurv = SimpleTrackedSurveillance('ac1_ac2_');
            this.ac1IntSenseEOIR = EOIRParametricModel('ac1IntEOIR_'); 
            this.ac1IntSenseADSB = SC228_AdsbModel('uas_adsb_'); 
            this.ac1IntSenseTrackedADSB = SC228_TrackedAdsbModel('uas_adsb_trckd_');
            this.ac1IntSenseAS = SC228_ActiveSurveillanceModel('uas_tcas_');
            this.ac1IntSenseTrackedAS = SC228_TrackedActiveSurveillanceModel('uas_tcas_trckd_');
            this.ac1IntSenseRadar = SC228_RadarModel('uas_rdr_');
            this.ac1IntSenseTrackedRadar = SC228_TrackedRadarModel('uas_rdr_trckd_');
            
            % Encounter file name
            this.encounterFile = 'sampleEncunters.mat';
        ...
        function theSim = setupEncounter( theSim, encNumber)
        ...
        function SimOut = runSimulink(obj, randSeed, varargin)
            if( nargin < 2 )
                SimOut = runSimulink@Simulation(obj);
            elseif (nargin<3)
                SimOut = runSimulink@Simulation(obj, randSeed);
            else
                SimOut = runSimulink@Simulation(obj, randSeed, varargin);
            end
